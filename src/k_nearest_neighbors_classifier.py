...
class KNearestNeighborsClassifier:
    def __init__(self, dataframe, prediction_column):
        self.data_dict = {key: value for key, value in dataframe.data_dict.items() if key != prediction_column}
    ...
    def compute_average_distances(self, observations):
        ...
        while len(distances) > 0:
            current_distances = [distance for distance in distances if distances[0][1] == distance[1]]
            average_distances[distances[0][1]] = (sum([distance[0] for distance in current_distances]) / len(current_distances))
            distances = [distance for distance in distances if distance not in current_distances]
        return average_distances        

    def classify(self, observations, k = 5):
        k_nearest_neighbors = [neighbor[1] for neighbor in self.nearest_neighbors(observations)[:k]]
        ...
        for _type_, count in count_of_types.items():
            if count > max_count[1]: max_count = [_type_, count]
            elif count == max_count[1] and max_count > 0: return min(self.compute_average_distances(observations).keys(), key=(lambda k: self.compute_average_distances(observations)[k]))
        return max_count[0]
